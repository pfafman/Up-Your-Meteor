...
Actions.prototype.deploy = function(option) {
  ...
  if (option == "-d") {
    ...
    console.log('Upload and deploy only'.blue, bundlePath); // + this.config.app);
...
Actions.prototype.logs = function() {
  ...
  for(var os in self.sessionsMap) {
    ...
    sessionsInfo.sessions.forEach(function(session) {
      ...
      if(os == 'linux') {
        var command = 'sudo tail ' + tailOptions + ' /var/log/meteor/' + self.config.appName + '.log';
      }
...
function whenAfterDeployed(buildLocation, option) {
  return function(error, summaryMaps) {
    ...
    if (option == "-s") {
      ...
      console.log("Build left at " + buildLocation.green);
    } else {
