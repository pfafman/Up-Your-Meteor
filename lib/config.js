...
exports.read = function() {
  ...
  if(fs.existsSync(mupJsonPath)) {
    ...
    if(typeof mupJson.debug === "undefined") {
      mupJson.debug = false;
    }
    if(typeof mupJson.serverOnly === "undefined") {
      mupJson.serverOnly = false;
    }
    ...
    if(!mupJson.servers || mupJson.servers.length == 0) {
      ...
      mupJson.servers.forEach(function(server) {
        ...
        var defaultEndpointUrl = format("http://%s:%s", server.host, server.env.PORT || 80);
