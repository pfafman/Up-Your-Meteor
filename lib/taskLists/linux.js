...
exports.setup = function(config) {
  ...
  //Configurations
  taskList.copy('Copy systemd service file', {
    src: path.resolve(TEMPLATES_DIR, 'meteor.service'),
    dest: config.appName + '.service',
    vars: {
      appName: config.appName
    }
  });

  taskList.execute('Configuring systemd ' + config.appName + '.service', {
    command: 'sudo mv ' + config.appName + '.service /lib/systemd/system/.; sudo chown root.root /lib/systemd/system/' + config.appName + '.service'
  });

  taskList.execute('Enable systemd  ' + config.appName + '.service', {
    command: 'sudo systemctl enable ' + config.appName 
  });
